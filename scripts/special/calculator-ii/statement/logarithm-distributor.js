...
export class CalculatorLogarithmDistributor extends CalculatorComponent {
    static distribute(input) {
        if (input instanceof CalculatorVariable)
            ...
            if (input.parameter instanceof CalculatorSingular)
                return input;
            else if (input.parameter instanceof CalculatorLogarithmFunction)
                return input;
            else if (input.parameter instanceof CalculatorFunction) {
                switch (input.parameter.operator) {
                    case CalculatorOperator.add: // ln(x +- y)
                        if (input.parameter.leftTerm instanceof CalculatorValue
                            && input.parameter.leftTerm.value.isEqualTo(MathNum.ZERO)) { // ln(0 + x) = ln(x)
                            return this.distribute(new CalculatorLogarithmFunction(input.parameter.rightTerm));
                        }
                        else if (input.parameter.rightTerm instanceof CalculatorValue
                            && input.parameter.rightTerm.value.isEqualTo(MathNum.ZERO)) { // ln(x + 0) = ln(x)
                    ...
                    case CalculatorOperator.subtract:
                        if (input.parameter.leftTerm instanceof CalculatorValue
                            && input.parameter.leftTerm.value.isEqualTo(MathNum.ZERO)) { // ln(0 - x) = ln(-1*x)
                            ...
                            if (invertedParameter)
                                ...
                                input.parameter = this.distribute(new CalculatorFunction(new CalculatorValue(-1), input.parameter.rightTerm, CalculatorOperator.multiply));
                                return input;
                            }
                        }
                        else if (input.parameter.rightTerm instanceof CalculatorValue
                    ...
                    case CalculatorOperator.multiply: // ln(xy) = ln(x) + ln(y)
                        return new CalculatorFunction(this.distribute(new CalculatorLogarithmFunction(input.parameter.leftTerm)), this.distribute(new CalculatorLogarithmFunction(input.parameter.rightTerm)), CalculatorOperator.add);
                    case CalculatorOperator.divide: // ln(x/y) = ln(x) - ln(y)
                        return new CalculatorFunction(this.distribute(new CalculatorLogarithmFunction(input.parameter.leftTerm)), this.distribute(new CalculatorLogarithmFunction(input.parameter.rightTerm)), CalculatorOperator.subtract);
                    case CalculatorOperator.exponent: // ln(x^y) = y * ln(x)
                        return new CalculatorFunction(this.distribute(input.parameter.rightTerm), this.distribute(new CalculatorLogarithmFunction(input.parameter.leftTerm)), CalculatorOperator.multiply);
                }
            }
            else
     ...
     */
    static invertTermSign(term) {
        if (term instanceof CalculatorValue)
            return new CalculatorValue(term.value.multiplyBy(MathNum.NEG_ONE));
        else if (term instanceof CalculatorVariable)
            return null;
        else if (term instanceof CalculatorLogarithmFunction) {
            const invertedParameter = this.invertTermSign(term.parameter);
            if (invertedParameter)
                return new CalculatorLogarithmFunction(invertedParameter);
            else
                return null;
        }
            ...
            switch (term.operator) {
                ...
                case CalculatorOperator.divide: { // -(x [*/] y) = (-x [*/] y) = (x [*/] -y)
                    const invertedLeft = this.invertTermSign(term.leftTerm);
                    if (invertedLeft)
                        return new CalculatorFunction(invertedLeft, term.rightTerm, term.operator);
                    else {
                        const invertedRight = this.invertTermSign(term.rightTerm);
                        if (invertedLeft)
                            return new CalculatorFunction(term.leftTerm, invertedRight, term.operator);
                        else
                            return null;
                    }
                }
                case CalculatorOperator.exponent: { // -(x ^ y)
                    ...
                    if (evaluatedExponent instanceof CalculatorValue &&
                        evaluatedExponent.value.isRealInteger() &&
                        evaluatedExponent.integerValue % 2 === 1) { // odd exponents can flip sign, even exponents cannot
                        return new CalculatorFunction(term.leftTerm, new CalculatorFunction(new CalculatorValue(-1), term.rightTerm, CalculatorOperator.multiply), CalculatorOperator.exponent);
                    }
                    else
                        return null;
                }
            ...
            throw new Error(`Bad input: ${term.print()}`);
    }
    static test() {
        const tester = new CalculatorTester('Logarithm Distributor', (input) => {
...
//# sourceMappingURL=logarithm-distributor.js.map