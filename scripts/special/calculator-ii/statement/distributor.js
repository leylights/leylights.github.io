...
export class CalculatorDistributor extends CalculatorComponent {
    ...
    static distributeRecurse(input, debug, depth) {
        ...
        if (input instanceof CalculatorFunction)
            this.log(debug, `Distribute start: ${space}${input.print(true, depth)}`);
                    ...
                    if (input.rightTerm instanceof CalculatorFunction && input.leftTerm instanceof CalculatorFunction) { // (w [+-*/^] x) [*/] (y [+-*/^] z)
                        if (input.operator === CalculatorOperator.multiply) { // (w [+-*/^] x) * (y [+-*/^] z)
                            switch (input.leftTerm.operator) {
                                ...
                                case CalculatorOperator.subtract: // (w [+-] x) * (y [+-*/^] z)
                                    switch (input.rightTerm.operator) {
                                        case CalculatorOperator.add:
                                        case CalculatorOperator.subtract: // (w [+-] x) * (y [+-] z)
                                            return exitRecurse(new CalculatorFunction(input.leftTerm, input.rightTerm.leftTerm, CalculatorOperator.multiply), new CalculatorFunction(input.leftTerm, input.rightTerm.rightTerm, CalculatorOperator.multiply), input.rightTerm.operator);
                                        case CalculatorOperator.multiply:
                                        case CalculatorOperator.divide: // (w [+-] x) * (y [*/] z)
                                            return exitRecurse(new CalculatorFunction(input.leftTerm.leftTerm, input.rightTerm, CalculatorOperator.multiply), new CalculatorFunction(input.leftTerm.rightTerm, input.rightTerm, CalculatorOperator.multiply), input.leftTerm.operator);
                                    ...
                                    switch (input.rightTerm.operator) {
                                        ...
                                        case CalculatorOperator.exponent: // (w ^ x) * (y ^ z)
                                            if (input.leftTerm.leftTerm.equals(input.rightTerm.leftTerm)) // same base: (w ^ x) * (w ^ z)
                                                return exitRecurse(input.leftTerm.leftTerm, new CalculatorFunction(input.leftTerm.rightTerm, input.rightTerm.rightTerm, CalculatorOperator.add), CalculatorOperator.exponent);
                                            else
                                                exitWithoutDistributing('(w ^ x) * (y ^ z) where w != y');
                                    }
                            ...
                            switch (input.leftTerm.operator) {
                                case CalculatorOperator.add:
                                case CalculatorOperator.subtract: // (w [+-] x) / (y [+-*/^] z)
                                    return exitRecurse(new CalculatorFunction(input.leftTerm.leftTerm, input.rightTerm, CalculatorOperator.divide), new CalculatorFunction(input.leftTerm.rightTerm, input.rightTerm, CalculatorOperator.divide), input.leftTerm.operator);
                                case CalculatorOperator.multiply:
                                case CalculatorOperator.divide: // 
                                    return exitWithoutDistributing('(w [*/] x) / (y [+-*/^] z)'); // no need to distribute
                                case CalculatorOperator.exponent: // w^x / (y [+-*/^] z)
                                    if (input.rightTerm.operator === CalculatorOperator.exponent && input.leftTerm.leftTerm.equals(input.rightTerm.leftTerm)) {
                                        ...
                                        input.leftTerm.leftTerm, new CalculatorFunction(input.leftTerm.rightTerm, input.rightTerm.rightTerm, CalculatorOperator.subtract), CalculatorOperator.exponent);
                                    }
                                    else
                                        return exitWithoutDistributing('(w ^ x) / (y [+-*/] z)'); // no need to distribute
                            }
                        ...
                        switch (input.leftTerm.operator) {
                            ...
                            case CalculatorOperator.subtract: // (x [+-] y) [*/] [3x] 
                                ...
                                return exitRecurse(new CalculatorFunction(input.leftTerm.leftTerm, input.rightTerm, input.operator), new CalculatorFunction(input.leftTerm.rightTerm, input.rightTerm, input.operator), input.leftTerm.operator);
                ...
                case CalculatorOperator.exponent:
                    if (input.leftTerm instanceof CalculatorFunction) {
                        if (input.leftTerm.operator === CalculatorOperator.multiply || input.leftTerm.operator === CalculatorOperator.divide) {
                            ...
                            return exitRecurse(input.leftTerm.leftTerm, new CalculatorFunction(input.leftTerm.rightTerm, input.rightTerm, CalculatorOperator.multiply), CalculatorOperator.exponent);
                        }
                        else
                            return exitWithoutDistributing('(j [+-] k) ^ (l +-*/^ m)');
                    }
                    else {
    ...
    static test() {
        ...
        tester.test('(((x * x) + (2 * x)) + ((3 * x) + (2 * 3)))*(x+4)', '(((((x * x) * x) + ((2 * x) * x)) + (((3 * x) * x) + ((2 * 3) * x))) + ((((x * x) * 4) + ((2 * x) * 4)) + (((3 * x) * 4) + ((2 * 3) * 4))))');
        tester.test('((x+2)*(x+3))*(x+4)', '(((((x * x) * x) + ((2 * x) * x)) + (((x * 3) * x) + ((2 * 3) * x))) + ((((x * x) * 4) + ((2 * x) * 4)) + (((x * 3) * 4) + ((2 * 3) * 4))))');
    }
}
