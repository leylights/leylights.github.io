...
export class CalculatorAssociator {
    /**
     * Pulls all variables to the right side of their disjunctive clause
     */
    static associate(input, debug) {
        this.log(debug, `---------- ASSOCIATE ----------`);
        return this.buildAssociatedTerm(input, debug);
    }
    /**
     * Invariant: nearestDisjunctiveClause is in the ancestor tree of input
     */
    static buildAssociatedTerm(input, debug) {
        const singulars = this.getValuesOfDisjuctiveClause(input);
        this.log(debug, `building associated term from ${input.print()}`);
        const values = {
            factors: [],
            divisors: [],
        };
        ...
        if (input instanceof CalculatorValue || input instanceof CalculatorVariable)
            ...
            switch (input.operator) {
                ...
                case CalculatorOperator.subtract:
                    input.leftTerm = this.buildAssociatedTerm(input.leftTerm, debug);
                    input.rightTerm = this.buildAssociatedTerm(input.rightTerm, debug);
                    return exit([input], []); // treat self as single multiplied clause
    ...
    static test() {
        const tester = new CalculatorTester((input, debug) => {
            return CalculatorAssociator.associate(new CalculatorParser(input).output, debug).print();
        });
