...
export class CalculatorCollector {
    ...
    - terms: ${terms.negatives.map((t) => t.print()).join(', ')} `);
        ...
        for (const entry of cws.Object.entries(filteredTerms)) {
            const positives = entry[1].positives;
            const negatives = entry[1].negatives;
            const coefficient = entry[1].coefficient;
            ...
            if (!output)
                ...
                output = new CalculatorFunction(output, result, positives.length > 0 ? CalculatorOperator.add : CalculatorOperator.subtract);
        }
