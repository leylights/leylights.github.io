...
function solve(equation, displaySteps) {
    ...
    if (rhs[0] instanceof mathFrac) {
        outputRhs += " \u2245 " + limitDecimals(rhs[0].numerator / rhs[0].denominator, 5);
    }
...
function solveFromHTML() {
    ...
    if (input === "") {
        ...
        document.getElementById("calcOutputField").value = solve(input);
}
/**
...
function EEA(a, b) {
     ...
     * @returns {Row}
     */
    function generateRow(last, slast) {
...
function divideSides(l, r, d) {
    ...
    if (typeof d === "number")
        ...
        fracDivisor = new mathFrac(d.denominator, d.numerator);
    else
        ...
        for (let i = 0; i < complicatedTerms.length; i++) {
            if (sortedComplicatedTerms.length === 0) {
                ...
                for (let j = 0; j < sortedComplicatedTerms.length; j++) {
                    if (arrayEquals(subArray(complicatedTerms[i], 1, complicatedTerms[i].length), subArray(sortedComplicatedTerms[j][0], 1, sortedComplicatedTerms[j][0].length))) {
...
function limitDecimalsOrExact(value, decimals) {
    if (value instanceof mathFrac)
        ...
        return limitDecimals(value, decimals);
}
