...
const myCanvas = {
    ...
    clear: function () {
        cws.fillRect(0, 0, this.canvas.width, this.canvas.height, "rgb(" + bgC + "," + bgC + "," + bgC + ")", myCanvas.context);
    }
};
function init(firstTime) {
    ...
    document.getElementById("axisStretchInput").placeholder = "" + customPreset.AXIS_MULTIPLIER * 100;
    document.getElementById("electricAmplitudeText").placeholder = "" + customPreset.BLUE_AMPLITUDE;
    document.getElementById("backAxesInput").checked = customPreset.BACK_AXES;
    document.getElementById("gridlinesText").placeholder = "" + customPreset.GRIDLINES;
    document.getElementById("vectorText").placeholder = "" + customPreset.VECTOR_SPACING;
    document.getElementById("xAngleInput").placeholder = "" + customPreset.X_ANGLE;
    document.getElementById("xLenInput").placeholder = "" + customPreset.X_LENGTH;
    document.getElementById("wavesText").placeholder = "" + Math.round((customPreset.Z_LENGTH / customPreset.WAVES) * 100) / 100;
    document.getElementById("zAngleInput").placeholder = "" + customPreset.Z_ANGLE;
    document.getElementById("zLenInput").placeholder = "" + Math.round(customPreset.Z_LENGTH / Math.cos((customPreset.Z_ANGLE - 90) / 180 * Math.PI));
    if (firstTime) {
        ...
        document.getElementById("hide-gridlines-button").addEventListener("click", () => {
            ...
            if (showGridlines)
                me.innerHTML = 'Hide gridlines';
            else
                me.innerHTML = 'Show gridlines';
        });
        ...
        document.getElementById("electric-hide-wave").addEventListener("click", () => {
            ...
            if (blueWave.show)
                me.innerHTML = 'Hide wave';
            else
                me.innerHTML = 'Show wave';
        });
        document.getElementById("electric-half-wave").addEventListener("click", () => {
            ...
            if (blueWave.bothHalves)
                me.innerHTML = 'Display half wave';
            else
                me.innerHTML = 'Display full wave';
        });
        ...
        document.getElementById("magnetic-half-wave").addEventListener("click", () => {
            ...
            if (redWave.bothHalves)
                me.innerHTML = 'Display half wave';
            else
                me.innerHTML = 'Display full wave';
        });
        document.getElementById("customizer-dropdown").addEventListener("click", () => {
            let me = document.getElementById("customizer-dropdown");
            for (let i = 1; i < me.parentElement.children.length; i++) {
                if (me.parentElement.children[i].style.display == 'none') {
                    me.parentElement.children[i].style.display = 'table';
                    me.parentElement.children[i - 1].style.borderBottom = '1px #555555 solid';
                }
                else {
                    me.parentElement.children[i].style.display = 'none';
                    me.parentElement.children[i - 1].style.borderBottom = 'none';
                }
            }
        });
...
function drawGridlines() {
    ...
    for (let x = (zAxis.endX - zAxis.startX) / preset.GRIDLINES; x <= len; x += (zAxis.endX - zAxis.startX) / preset.GRIDLINES) {
        ...
        c.lineTo(Math.round(x + zAxis.startX), Math.round(getYComponent(x) + (yAxis.endY - yAxis.startY) * preset.AXIS_MULTIPLIER));
        c.stroke();
        c.beginPath();
        // x-axis
        c.moveTo(startPoint.x, startPoint.y);
        c.lineTo(Math.round(x + zAxis.startX + (xAxis.endX - xAxis.startX) * 1), Math.round(getYComponent(x) + (xAxis.endY - xAxis.startY) * 1));
        c.stroke();
        c.beginPath();
    }
    c.stroke();
...
function drawSine(axis, wave) {
    ...
    for (let x = 0; x <= len; x++) {
        ...
        c.lineTo(Math.round(finalX), Math.round(finalY));
    }
    c.stroke();
    c.beginPath();
}
...
function findOriginAngle(useX) {
    ...
    let angleYOZ = Math.acos((Math.pow(lineYZ, 2) - Math.pow(lineOZ, 2) - Math.pow(lineOY, 2))
        /
            (-2 * lineOZ * lineOY));
...
function generatePresetButtons() {
    ...
    for (let i = 0; i < PRESETS.length; i++) {
        let data = PRESETS[i];
        let newButton = original.cloneNode(true);
...
function handleDropdownOpener(opener) {
    for (let i = 1; i < opener.parentElement.children.length; i++) {
        if (opener.parentElement.children[i].style.display == 'none') {
            opener.parentElement.children[i].style.display = 'table';
            opener.parentElement.children[i - 1].style.borderBottom = '1px #555555 solid';
        }
        else {
            opener.parentElement.children[i].style.display = 'none';
            opener.parentElement.children[i - 1].style.borderBottom = 'none';
        }
    }
}
/**
...
function Line(x1, y1, x2, y2, w, c, angle, lenFixed) {
    ...
    if (!this.lengthIsFixed)
        this.recalcCosmeticEnd();
}
/**
...
function reAngle() {
    ...
    if (now < lastWaveChange + 50) { // USED FOR DEBUGGER Z-LENGTH CHANGES
        ...
        zAxis = new Line(graphOrigin.x, graphOrigin.y, graphOrigin.x + preset.Z_LENGTH, graphOrigin.y + preset.Z_LENGTH * Math.tan((angles.z - 90) / 180 * Math.PI), 2, "black", angles.z, true);
    // cosmetic axes
 ...
 *
 */
function resizeControlsBox() {
    const controlsBox = document.getElementById('controls-inner'), opener = document.getElementById('controlsOpener');
    controlsBox.style.height = (opener.getBoundingClientRect().y - controlsBox.getBoundingClientRect().y) + 'px';
}
init(true);
//# sourceMappingURL=emwaves.js.map