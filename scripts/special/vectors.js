...
function init(firstTime, fullRestart) {
    ...
    if (firstTime) {
        ...
        for (let i = 0; i < dropSpaces.length; i++) {
            dropSpaces[i].addEventListener("drop", (evt) => { dragDrop(evt, evt.target); });
...
function buildInfo(vector) {
    ...
    let inner = "<div class='vectorColour' draggable = 'true' onclick='setVectorVisibility(this.parentElement)' \
  ...
  <div class = 'closeInfoTab'>x</div> ";
    ...
    info.querySelector(".vectorColour").addEventListener("dragstart", (evt) => { dragStart(evt.target.parentElement); });
    ...
    for (let i = 0; i < info.children[1].children[0].children.length; i++) {
        info.children[1].children[0].children[i].value = cws.Object.entries(vector.components)[i][1] + '';
    }
...
function drawScale() {
    ...
    for (let z = 1; z < (myCanvas.canvas.width / Math.cos(zAxis.angle) / 2) / unitLength; z++) { // negative (looks to be in q1)
        ...
        if (z % scaleInterval == 0) // make bold
            c = altScaleColour;
        myCanvas.drawLine(graphOrigin.x + z * zScalePoint.x - ((tickLength / 2) * Math.sin(zAxis.angle)), graphOrigin.y - z * zScalePoint.y - ((tickLength / 2) * Math.cos(zAxis.angle)), graphOrigin.x + z * zScalePoint.x + ((tickLength / 2) * Math.sin(zAxis.angle)), graphOrigin.y - z * zScalePoint.y + ((tickLength / 2) * Math.cos(zAxis.angle)), c, 1); // scale marker
    }
...
function updateVectors() {
    for (let i = 0; i < vectors.length; i++) {
        ...
        vectors[i].setComponents(parseFloat(data[0].value), parseFloat(data[1].value), parseFloat(data[2].value));
    }
}
/**
 * The Vector object
