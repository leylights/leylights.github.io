...
var myCanvas = {
    ...
    start: function () {
        ...
        this.canvas.setAttributeNode(cws.betterCreateAttr("oncontextmenu", "return false;"));
        ...
        myCanvas.canvas.addEventListener("touchend", function (evt) {
            let canvasArea = myCanvas.canvas.getBoundingClientRect();
        ...
        myCanvas.canvas.addEventListener("mousemove", function (evt) {
            let canvasArea = myCanvas.canvas.getBoundingClientRect();
            myCanvas.mouseX = evt.clientX - canvasArea.left;
            myCanvas.mouseY = evt.clientY - canvasArea.top;
        }, false);
        myCanvas.canvas.addEventListener("contextmenu", function (evt) {
            let canvasArea = myCanvas.canvas.getBoundingClientRect();
            myCanvas.mouseX = evt.clientX - canvasArea.left;
            myCanvas.mouseY = evt.clientY - canvasArea.top;
            myCanvas.clickTime = now;
    ...
    clear: function () {
        cws.fillRect(0, 0, this.canvas.width, this.canvas.height, this.bg, this.context);
...
function tile(x, y, w, h, img) {
    ...
    this.draw = function (img) {
        ...
        myCanvas.context.rotate((this.permarotation + this.rotation) * (Math.PI / 180));
        if (showRed && this.isPowered)
            ...
            drawImg(this.img, -this.w / 2, -this.h / 2, this.w, this.h);
        if (this.hovering)
...
function init(inComp) {
    ...
    try {
        if (comp) {
            if (localStorage.getItem("wiresCompScore") === null)
                score = 0;
            else
                score = parseInt(localStorage.getItem("wiresCompScore"));
        }
        else {
            if (localStorage.getItem("wiresScore") === null)
                score = 0;
            else
                score = parseInt(localStorage.getItem("wiresScore"));
        }
    }
    catch (e) {
        score = 0;
...
function main() {
    ...
    redraw();
    doThings();
    myCanvas.canvas.style.backgroundColor = document.getElementById("main").style.backgroundColor;
    ...
    switch (switchState) {
        case 1:
            if (bgC === 255) {
                ...
                c = Math.floor((now - bgSwitchTime) / (1.96078431373 * 1));
            if (c > 255)
                c = 255;
            bgC = c;
            break;
        case -1:
            if (bgC === 0) {
                ...
                c = 255 - Math.floor((now - bgSwitchTime) / (1.96078431373 * 1));
            if (c < 0)
                c = 0;
            bgC = c;
            break;
    ...
    if (isWon === 1) {
        ...
        if (typeof localStorage !== "undefined") {
            if (comp)
                localStorage.setItem("wiresCompScore", score + "");
            else
                localStorage.setItem("wiresScore", score + "");
        }
        if (comp) {
...
function getIn() {
    ...
    if (myCanvas.recentClick) {
        ...
        if (isClicked(solveButton.x, solveButton.y, solveButton.w, solveButton.h) &&
        ...
        if (!comp) {
            ...
            else if (isClicked(80 + tiles[0][0].x + compw, tiles[0][0].y - 40, 35, 35)) {
                makeNewPuzzle();
            }
            else if (isClicked(120 + tiles[0][0].x + compw, tiles[0][0].y - 40, 35, 35)) {
                restrictSize = !restrictSize;
            }
            else if (isClicked(160 + tiles[0][0].x + compw, tiles[0][0].y - 40, 35, 35)) {
                switch (tileW) {
                    case 64:
                        tileW = 48;
                        break;
    ...
    if (isHovering(solveButton.x, solveButton.y, solveButton.w, solveButton.h)) {
        ...
        solveButton.c = "#000022";
    if (((isHovering(40 + tiles[0][0].x + compw, tiles[0][0].y - 40, 35, 35) ||
...
function redraw() {
    ...
    if (comp)
        ...
        compw = 0;
    myCanvas.clear();
    rect(tiles[0][0].x - 5, tiles[0][0].y - 5, tiles[0][0].w * tiles.length + 10, tiles[0][0].w * tiles[0].length + 10, "rgb(" + (255 - bgC) + "," + (255 - bgC) + "," + (255 - bgC) + ")"); // outline
    rect(tiles[0][0].x, tiles[0][0].y, tiles[0][0].w * tiles.length, tiles[0][0].w * tiles[0].length, "black"); // backdrop
    if (!comp) {
        rect(tiles[0][0].x - 5, tiles[0][0].y - 45, 205 + compw, 45, "rgb(" + (255 - bgC) + "," + (255 - bgC) + "," + (255 - bgC) + ")"); // top HUD outline
    }
    else {
        rect(tiles[0][0].x - 5, tiles[0][0].y - 45, 125 + compw, 45, "rgb(" + (255 - bgC) + "," + (255 - bgC) + "," + (255 - bgC) + ")"); // top HUD outline
    }
    if (comp) {
    ...
    drawText(score, 17 + tiles[0][0].x + compw, tiles[0][0].y - 15, "white", true, 24, "Encode Sans");
    if (!comp) {
        drawText("?", 57 + tiles[0][0].x + compw, tiles[0][0].y - 15, "white", true, 24, "Encode Sans");
        drawText("\u27F2", 97 + tiles[0][0].x + compw, tiles[0][0].y - 15, "white", true, 24, "Encode Sans");
        if (!restrictSize)
            drawImg(sprites[0], 121 + tiles[0][0].x + 6 + compw, tiles[0][0].y - 34, 21, 21);
        else
            drawText("\u221E", 137 + tiles[0][0].x + compw, tiles[0][0].y - 15, "white", true, 24, "Encode Sans");
    }
    else {
        if (tiles[startTile[0]][startTile[1]].isPowered)
            drawText(Math.floor((deadline - stopTime) / 1000), 97 + tiles[0][0].x + compw, tiles[0][0].y - 15, "white", true, 24, "Encode Sans");
        else
            drawText(Math.floor((deadline - now) / 1000), 97 + tiles[0][0].x + compw, tiles[0][0].y - 15, "white", true, 24, "Encode Sans");
    }
    {
        let sz;
        switch (tileW) {
        ...
        if (comp)
            drawText(sz, 57 + tiles[0][0].x + compw, tiles[0][0].y - 15, "white", true, 24, "Times New Roman");
        else
            drawText(sz, 177 + tiles[0][0].x + compw, tiles[0][0].y - 15, "white", true, 24, "Times New Roman");
    }
    ...
    if (tiles[startTile[0]][startTile[1]].isPowered) {
        rect(startPos.x - 5, startPos.y - 5, startPos.w + 5, startPos.h + 10, "rgb(" + (255 - bgC) + "," + (255 - bgC) + "," + (255 - bgC) + ")");
        rect(startPos.x, startPos.y, startPos.w, startPos.h, startPos.c);
    }
    else {
        rect(startPos.x - 10, startPos.y - 5, startPos.w + 5, startPos.h + 10, "rgb(" + (255 - bgC) + "," + (255 - bgC) + "," + (255 - bgC) + ")");
        rect(startPos.x - 5, startPos.y, startPos.w, startPos.h, startPos.c);
    }
    if (endPos.w > endPos.h)
        rect(endPos.x - 5, endPos.y, endPos.w + 10, endPos.h + 5, "rgb(" + (255 - bgC) + "," + (255 - bgC) + "," + (255 - bgC) + ")");
    else
        rect(endPos.x, endPos.y - 5, endPos.w + 5, endPos.h + 10, "rgb(" + (255 - bgC) + "," + (255 - bgC) + "," + (255 - bgC) + ")");
    rect(endPos.x, endPos.y, endPos.w, endPos.h, endPos.c);
    rect(solveButton.x - 5, solveButton.y, solveButton.w + 10, solveButton.h + 5, "rgb(" + (255 - bgC) + "," + (255 - bgC) + "," + (255 - bgC) + ")");
    rect(solveButton.x, solveButton.y, solveButton.w, solveButton.h, solveButton.c);
    if (comp) {
        rect(solveButton.x + solveButton.w + 5, solveButton.y - 5, 95, 45, "rgb(" + (255 - bgC) + "," + (255 - bgC) + "," + (255 - bgC) + ")");
        ...
        drawText("TOP: " + hs, solveButton.x + solveButton.w + 5 + 45, solveButton.y + 25, "white", true, 24, "Encode Sans");
    }
    drawText("\u2713", solveButton.x + solveButton.w / 2, solveButton.y + solveButton.h * 0.8, "white", true, Math.floor(solveButton.w * 0.75), "Times New Roman");
}
...
function resetBounds() {
    ...
    maxH = 0;
    while (true) {
        if ((maxW + 1) * tileW + 80 > myCanvas.canvas.width ||
            (restrictSize && maxW >= 9))
            break;
        maxW++;
    }
    while (true) {
        if ((maxH + 1) * tileW + (60 + tileW) > myCanvas.canvas.height ||
            (restrictSize && maxH >= 6))
            break;
        maxH++;
    }
...
function buildTiles(columns, rows) {
    ...
    solveButton = {
        ...
        y: tiles[tiles.length - 1][tiles[0].length - 1].y +
...
function buildPath(x, y, start, iteration) {
    ...
    if (start) {
        try {
            ...
            console.log("buildPath(): Cannot read property 'exits' of undefined at " +
                x +
                " " +
                y);
        }
...
function makeNewPuzzle() {
    resetBounds();
    buildTiles(Math.ceil(Math.random() * (maxW - minW)) + minW, Math.ceil(Math.random() * (maxH - minH)) + minH);
    if (comp)
        calcTime();
}
function topTile(x) {
...
//       "rgba(0,0,200,0.5)";
//   }
//# sourceMappingURL=wires-v1.js.map