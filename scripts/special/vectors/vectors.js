...
const tickLength = 8;
const scaleColour = "#999999";
const altScaleColour = "#000000";
let currentDraggedObject;
let infoTabs = 0;
const showScaleNumbers = true;
class Vectors {
    static init(firstTime, fullRestart) {
        ...
        this.vectors.forEach((v) => v.resetCosmetics());
        if (firstTime) {
            ...
            Array.from(document.getElementsByClassName("vectorDropArea")).forEach((dropArea) => {
                dropArea.addEventListener("drop", (evt) => { Vectors.dragDrop(evt, evt.target); });
    ...
    static buildInfo(vector) {
        ...
        const info = Molasses.createElement({
            type: 'div',
            classList: 'infoTab',
            id: "infoTab" + infoTabs,
        });
    ...
    <p> x: <input></input> y: <input></input> z: <input></input> </p>
        ...
        info.querySelector(".vectorColour").addEventListener("dragstart", (evt) => { Vectors.dragStart(evt.target.parentElement); });
    ...
    static calculationInput(type) {
        ...
        switch (type) {
            ...
            case "tsp": {
                ...
                const cross = Vectors.makeCrossProduct(b, c, false);
     ...
     */
    static createVector() {
        const data = Array.from(document.getElementById("vectorCreator").children[1].children[0].children[0].children);
     ...
     */
    static dragDrop(event, dropZone) {
        // dropping the vectorColour
        console.log(currentDraggedObject);
        dropZone.style.backgroundColor = currentDraggedObject.querySelector('.vectorColour').style.backgroundColor;
    ...
    static drawScale() {
        ...
        for (let z = 1; z < (Vectors.canvas.width / Math.cos(zAxis.angle) / 2) / Vectors.unitLength; z++) { // negative (looks to be in q1)
            ...
            if (z % Vectors.scaleInterval == 0) // make bold
                c = altScaleColour;
            Vectors.canvas.drawLine(graphOrigin.x + z * zScalePoint.x - ((tickLength / 2) * Math.sin(zAxis.angle)), graphOrigin.y - z * zScalePoint.y - ((tickLength / 2) * Math.cos(zAxis.angle)), graphOrigin.x + z * zScalePoint.x + ((tickLength / 2) * Math.sin(zAxis.angle)), graphOrigin.y - z * zScalePoint.y + ((tickLength / 2) * Math.cos(zAxis.angle)), c, 1); // scale marker
        }
     ...
     */
    static getScalarProjection(a, b) {
     ...
     */
    static makeCrossProduct(a, b, createVector) {
     ...
     * @param {Vector} b Vector b
     */
    static makeVectorProjection(a, b) {
     ...
     */
    static newVector(x1, y1, z1, x2, y2, z2, colour, width) {
        if (colour == undefined)
            colour = COLOURS[infoTabs % COLOURS.length];
        const v = new Vector(x1, y1, z1, x2, y2, z2, colour, width);
        Vectors.buildInfo(v);
    ...
    static setScale(newNumTicks) {
        ...
        Vectors.vectors.forEach((v) => v.resetCosmetics());
    }
    /**
    ...
    static setVectorVisibility(el) {
        ...
        const numberElement = el.querySelector('.vectorColour');
    ...
    static updateVectors() {
        Vectors.vectors.forEach((v) => {
            const tab = v.getInfoTab();
            const data = Array.from(tab.querySelectorAll('input'));
            v.setComponents(parseFloat(data[0].value), parseFloat(data[1].value), parseFloat(data[2].value));
        });
    }
}
Vectors.vectors = [];
Vectors.recentClick = false;
Vectors.scaleInterval = 5; // number at which to mark the scale
Vectors.numTicks = 40;
Vectors.drawnTicks = 1; // every *drawnticks*th tick will be drawn
Vectors.canvas = new Canvas({
...
class Vector {
     ...
     */
    constructor(x1, y1, z1, x2, y2, z2, colour, width) {
        this.isVisible = true;
    ...
    draw() {
        if (!this.isVisible)
            return;
        Vectors.canvas.drawLine(this.cosmeticgraphOrigin.x, this.cosmeticgraphOrigin.y, this.cosmeticgraphOrigin.x + this.cosmeticComponents.x, this.cosmeticgraphOrigin.y + this.cosmeticComponents.y, this.colour, VECTOR_WIDTH);
