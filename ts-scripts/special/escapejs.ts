...
var myCanvas = {
  ...
  start: function () {
    ...
    if (document.body.clientHeight < 600)
      ...
      } else this.canvas.height = document.body.clientHeight;
...
function Component(x, y, w, h, c, type?, name?, isImage?: boolean) {
  this.x = x;
  this.y = y;
  this.w = w;
  this.h = h;
  ...
  this.move = function () {
    if (Molasses.orEquals(this.name, ["gunner", "laser", "heavy", "baller"])) {
      if (this.y + this.h / 2 < player.y + player.h / 2)
        this.yvel += this.speed / 10;
      else if (this.y + this.h / 2 > player.y + player.h / 2)
        this.yvel -= this.speed / 10;
      if (this.x < myCanvas.canvas.width * 0.75) this.xvel += this.speed / 20;
  ...
  this.resets = function () {
    ...
    if (typeof this.maxSpeedX === "number") {
      if (this.xvel < 0.25 && this.xvel > 0.25) this.xvel = 0;
      else if (this.xvel > this.maxSpeedX) this.xvel = this.maxSpeedX;
      else if (this.xvel < -this.maxSpeedX) this.xvel = -this.maxSpeedX;

      if (this.yvel < 0.25 && this.yvel > 0.25) this.yvel = 0;
      else if (this.yvel > this.maxSpeedY) this.yvel = this.maxSpeedY;
      else if (this.yvel < -this.maxSpeedY) this.yvel = -this.maxSpeedY;
    }
  };
...
function main() {
  ...
  if (!playing) {
    if (upgrading) {
      ...
      if (
        isClicked(200, 215, 120, 120) ||
        (myCanvas.keys && myCanvas.keys[81])
      ) {
        player.maxHP += 40;
        upgrading = false;
      } else if (
        isClicked(480, 215, 120, 120) ||
      ...
      for (var i = 0; i < controls.length; i++) {
        if (controls[i] === "white") break;
        else if (i === controls.length - 1) thisRound++;
      }
      if (mobile) {
        thisRound++;
      }
      ...
      if (player.x >= 100) {
        if (player.hp !== -9000) {
          if (thisRound != 1 || player.hp != player.maxHP) thisRound++;
          if (maxObs < 10) maxObs++;
          if (thisRound % 3 === 0) minEn++;
          maxEn++;
        }
...
function redraw() {
  ...
  for (var i = 0; i < bgx.length; i++) if (bgx[i] < -800) bgx[i] += 800;
  ...
  if (mobile) {
    if (player.y + player.h > myCanvas.canvas.height - 60) {
      ...
      if (player.weapon === "gunner") rect(290, player.y + player.h + 3, 54, 54, "#00c800");
      drawImg(sprites[3][0], 292, player.y + player.h + 5, 50, 50);
      if (player.weapon === "laser")
        if (player.shootTimer === 0) rect(346, player.y + player.h + 3, 54, 54, "#00c800");
        else rect(346, player.y + player.h + 3, 54, 54, "#ffe100");
      drawImg(sprites[3][1], 348, player.y + player.h + 5, 50, 50);
      if (player.weapon === "heavy") rect(401, player.y + player.h + 3, 54, 54, "#00c800");
      drawImg(sprites[3][2], 403, player.y + player.h + 5, 50, 50);
      if (player.weapon === "baller")
        if (player.shootTimer === 0) rect(456, player.y + player.h + 3, 54, 54, "#00c800");
        else rect(456, player.y + player.h + 3, 54, 54, "#ffe100");
      drawImg(sprites[3][3], 458, player.y + player.h + 5, 50, 50);
    } else {
      rect(288, myCanvas.canvas.height - 60, 225, 60, "white");
      if (player.weapon === "gunner") rect(290, myCanvas.canvas.height - 57, 54, 54, "#00c800");
      drawImg(sprites[3][0], 292, myCanvas.canvas.height - 55, 50, 50);
      if (player.weapon === "laser")
        if (player.shootTimer === 0) rect(346, myCanvas.canvas.height - 57, 54, 54, "#00c800");
        else rect(346, myCanvas.canvas.height - 57, 54, 54, "#ffe100");
      drawImg(sprites[3][1], 348, myCanvas.canvas.height - 55, 50, 50);
      if (player.weapon === "heavy") rect(401, myCanvas.canvas.height - 57, 54, 54, "#00c800");
      drawImg(sprites[3][2], 403, myCanvas.canvas.height - 55, 50, 50);
      if (player.weapon === "baller")
        if (player.shootTimer === 0) rect(456, myCanvas.canvas.height - 57, 54, 54, "#00c800");
        else rect(456, myCanvas.canvas.height - 57, 54, 54, "#ffe100");
      drawImg(sprites[3][3], 458, myCanvas.canvas.height - 55, 50, 50);
    }
    if (isClicked(290, myCanvas.canvas.height - 57, 50, 54)) player.weapon = "gunner";
    else if (isClicked(348, myCanvas.canvas.height - 57, 50, 54)) player.weapon = "laser";
    else if (isClicked(403, myCanvas.canvas.height - 57, 50, 54)) player.weapon = "heavy";
    else if (isClicked(458, myCanvas.canvas.height - 57, 50, 54)) player.weapon = "baller";
    ...
    if (player.weapon === "gunner") rect(3, 3, 54, 54, "#00c800");
    drawImg(sprites[3][0], 5, 5, 50, 50);
    if (player.weapon === "laser")
      if (player.shootTimer === 0) rect(3, 58, 54, 54, "#00c800");
      else rect(3, 58, 54, 54, "#ffe100");
    drawImg(sprites[3][1], 5, 60, 50, 50);
    if (player.weapon === "heavy") rect(3, 113, 54, 54, "#00c800");
    drawImg(sprites[3][2], 5, 115, 50, 50);
    if (player.weapon === "baller")
      if (player.shootTimer === 0) rect(3, 168, 54, 54, "#00c800");
      else rect(3, 168, 54, 54, "#ffe100");
    drawImg(sprites[3][3], 5, 170, 50, 50);
  } else {
    rect(player.x - 60, 0, 60, 225, "white");

    if (player.weapon === "gunner") rect(player.x - 57, 3, 54, 54, "#00c800");
    drawImg(sprites[3][0], player.x - 55, 5, 50, 50);
    if (player.weapon === "laser")
      if (player.shootTimer === 0) rect(player.x - 57, 58, 54, 54, "#00c800");
      else rect(player.x - 57, 58, 54, 54, "#ffe100");
    drawImg(sprites[3][1], player.x - 55, 60, 50, 50);
    if (player.weapon === "heavy") rect(player.x - 57, 113, 54, 54, "#00c800");
    drawImg(sprites[3][2], player.x - 55, 115, 50, 50);
    if (player.weapon === "baller")
      if (player.shootTimer === 0) rect(player.x - 57, 168, 54, 54, "#00c800");
      else rect(player.x - 57, 168, 54, 54, "#ffe100");
    drawImg(sprites[3][3], player.x - 55, 170, 50, 50);
  }

  if (isClicked(5, 5, 50, 50)) player.weapon = "gunner";
  else if (isClicked(5, 60, 50, 50)) player.weapon = "laser";
  else if (isClicked(5, 115, 50, 50)) player.weapon = "heavy";
  else if (isClicked(5, 170, 50, 50)) player.weapon = "baller";
  ...
  if (mobile) {
    if (player.y < 50) {
      ...
      rect(
        200,
        player.y - 25,
        Math.round((now - roundStart) / 1000 / 60 * myCanvas.canvas.width * 0.75),
        25,
        "cyan"
      );
    ...
    rect(
      0,
      580,
      Math.round((now - roundStart) / 1000 / 60 * myCanvas.canvas.width),
      20,
      "cyan"
    );
    drawText(
...
function getInput() {
  if (mobile) {
    if (circleIsClicked(0, myCanvas.canvas.height - 200, 200, true)) isFiring = !isFiring;
    ...
    if (thisRound === 0) {
      ...
      if (myCanvas.keys && myCanvas.keys[32] && player.shootTimer === 0)
        // space
        controls[4] = "#00ff00";
      if (now < myCanvas.clickTime + 64) controls[5] = "#00ff00";
...
function playerChecks() {
  ...
  if (player.x < 0) {
    player.hp -= 0.25;
    player.xvel += 3;
  } else if (player.x + player.w > myCanvas.canvas.width) player.x -= 3;
  else if (player.x + player.w > myCanvas.canvas.width * 0.875) player.x -= 2;
  else if (player.x + player.w > myCanvas.canvas.width * 0.75) player.x -= 1;
  if (player.y < 0) player.yvel = 3;
  else if ((mobile && player.y + player.h > myCanvas.canvas.height + 50) || (!mobile && player.y + player.h > myCanvas.canvas.height - 50)) player.yvel = -3;
...
function obsChecks() {
  ...
  if (obs.length < maxObs && Math.random() < 0.01 && en.length > 0) spawnObs();
}
...
function enChecks() {
  for (var i = 0; i < en.length; i++) {
    ...
    if (Math.random() < 0.1) spawnProj(e);
    else if (Math.random() < 0.5 && e.name === "heavy") spawnProj(e);
  ...
  if (!stopSpawns) {
    ...
    } else if (Math.random() < 0.0003) spawnEn("gunner");
  }
}
...
function projChecks() {
  for (var i = 0; i < proj.length; i++) {
    ...
    if (p.w === 20 && p.h === 20 && typeof en[0] !== "undefined") {
      if (p.user === "player") pathfind(p, en[0]);
      else pathfind(p, player);
    } else p.xvel = p.speed;
    ...
    if (p.findCollision(player) !== "none" && p.user !== "player") {
      player.hp -= p.dmg;
      if (mobile) player.hp -= p.dmg * -0.2;
      if (p.w === 20) p.hp -= 10;
      else if (p.user === "laser") p.hp = 0;
      else p.hp--;
    }
...
function spawnObs() {
  ...
  var newObs = new Component(
    myCanvas.canvas.width,
    Math.ceil(Math.random() * myCanvas.canvas.height - 50) - h / 2,
    w,
    h,
    c
  );
...
function spawnEn(name) {
  ...
  switch (name) {
    case "gunner":
      ...
      break;
    case "laser":
      if (maxEn < en.length) newEn.setEVars(1, [2, 3]);
      else newEn.setEVars(3, [4, 6]);
      hp = 4;
...
function spawnProj(user) {
  ...
  switch (user.weapon) {
    case "gunner":
      ...
      if (user === player) {
        ...
        user.shootTimer += 25;
        if (user.x < player.x) var colour = sprites[1][8];
        else var colour = sprites[1][4];
      }
      break;
    case "laser":
      w = 100;
      ...
      if (user === player) {
        ...
        if (user.x < player.x) var colour = sprites[1][9];
        else var colour = sprites[1][5];
      }
      break;
    case "heavy":
      w = 8;
      ...
      if (user === player) {
        ...
        if (user.x < player.x) var colour = sprites[1][10];
        else var colour = sprites[1][6];
      }
      break;
    case "baller":
      w = 20;
      ...
      if (user === player) {
        ...
        if (user.x < player.x) var colour = sprites[1][11];
        else var colour = sprites[1][7];
      }
      break;
  }
  ...
  if (user.weapon === "baller") newProj.setEVars(-speed, [speed, speed]);
  else if (user.x > player.x) newProj.setEVars(-speed, [speed, 0]);
  else newProj.setEVars(speed, [speed, 0]);
...
function circleCollide(c, r) {
  ...
  if (c.findCollision(r) !== "none") {
    ...
    if (
      Math.sqrt(
        Math.pow(Math.abs(c.x - r.x + r.w), 2) +
...
function drawEntry() {
  ...
  if (myCanvas.canvas.height >= 600) {
    ...
    if (controls[5] === "white") drawImg(sprites[3][6], 540, 275, 120, 175);
    else drawImg(sprites[3][7], 540, 275, 120, 175);
  }

  rect(150, myCanvas.canvas.height - 80, 500, 60, "white");
  drawText(
    "Press here for mobile edition",
    400,
    myCanvas.canvas.height - 40,
    "black",
    true,
    28,
    "Consolas"
...
function drawText(text, x, y, colour, centered, size, type) {
  ...
  if (type != null) myCanvas.context.font = size + "px " + type;
  if (centered) myCanvas.context.textAlign = "center";
  else myCanvas.context.textAlign = "left";
  myCanvas.context.fillText(text, x, y);
}

function drawImg(img, x, y, w, h) {
  if (h === null) myCanvas.context.drawImage(img, x, y);
  else myCanvas.context.drawImage(img, x, y, w, h);
}
...
function drawRotatedImg(img, x, y, w, h, angle) {
  ...
  else myCanvas.context.drawImage(img, -w / 2, -h / 2, w, h);
  myCanvas.context.rotate(-angle * (Math.PI / 180));
  myCanvas.context.translate(-x - img.width / 2, -y - img.height / 2);
}
