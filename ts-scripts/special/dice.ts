...
const MIN_ROLLS: number = 8;

var inputBox: HTMLInputElement = (document.getElementById("diceIn") as HTMLInputElement);
var lastRoll: string = "";
...
function diceInit() {
  ...
  document.getElementById("colour-slider").oninput = function () {
    let dice: HTMLCollection = document.getElementsByClassName("dice");
    let me: HTMLInputElement = (document.getElementById("colour-slider") as HTMLInputElement);

    for (let i = 0; i < dice.length; i++) {
      (dice[i] as HTMLElement).style.filter = "drop-shadow(0px 2px 6px black) sepia(1) hue-rotate(" + me.value + "deg) saturate(1.25)";
    }
  ...
  //dice
  let normalTable: HTMLElement = document.getElementById("base-dice");
...
export function roll(input: string) {
  let amt: number = parseInt(input);
  let type: number = parseInt(input.substring((amt + "").length + 1));
  if (isNaN(type)) {
    inputBox.value = "";
  ...
  for (let i = 0; i < rolls.length; i++)
    if (i === 0) sumT = sumT + rolls[i];
    else sumT = sumT + ", " + rolls[i];
...
function rollDie(type: number, element: HTMLElement, iter?: number) {
  if (typeof iter == "undefined")
    iter = 0;
  setTimeout(function () {
    if (iter > type && iter > MIN_ROLLS || iter > 30) return null;
...
function rollD100(iter: number | undefined) {
  if (typeof iter == "undefined")
    iter = 0;
  setTimeout(function () {
    if (iter > MIN_ROLLS || iter > 30) return null;
...
function openDiceColourSelect(opening: boolean) {
...
function openHelp() {
  let q: HTMLElement = document.getElementById("q");
  let qs = q.style;
