...
export class CalculatorDistributor extends CalculatorComponent {
  ...
  private static distributeRecurse(input: CalculatorTerm, debug?: boolean): CalculatorTerm {
    this.recursiveCalls++;
    if (this.recursiveCalls > 60) throw new Error('LOOP.');
    ...
    if (input instanceof CalculatorValue) return input;
      ...
      const exitWithoutDistributing = () => {
        return new CalculatorFunction(
          this.distributeRecurse(input.leftTerm, debug),
          this.distributeRecurse(input.rightTerm, debug),
          input.operator);
      }
          ...
          if (input.rightTerm instanceof CalculatorFunction && input.leftTerm instanceof CalculatorFunction) { // (w [+-*/^] x) [*/] (y [+-*/^] z)
            if (input.operator === CalculatorOperator.multiply) { // (w [+-*/^] x) * (y [+-*/^] z)
              switch (input.leftTerm.operator) {
                ...
                case CalculatorOperator.subtract: // (w [+-] x) * (y [+-*/^] z)
                  switch (input.rightTerm.operator) {
                    ...
                    case CalculatorOperator.subtract: // (w [+-] x) * (y [+-] z)
                      return new CalculatorFunction(
                        this.distributeRecurse(new CalculatorFunction(input.leftTerm, input.rightTerm.leftTerm, CalculatorOperator.multiply), debug),
                        this.distributeRecurse(new CalculatorFunction(input.leftTerm, input.rightTerm.rightTerm, CalculatorOperator.multiply), debug),
                        input.rightTerm.operator
                      );
                    case CalculatorOperator.multiply:
                    case CalculatorOperator.divide: // (w [+-] x) * (y [*/] z)
                      return new CalculatorFunction(
                        this.distributeRecurse(new CalculatorFunction(input.leftTerm.leftTerm, input.rightTerm, CalculatorOperator.multiply), debug),
                        this.distributeRecurse(new CalculatorFunction(input.leftTerm.rightTerm, input.rightTerm, CalculatorOperator.multiply), debug),
              ...
              switch (input.leftTerm.operator) {
                ...
                case CalculatorOperator.subtract: // (w [+-] x) / (y [+-*/^] z)
                  return new CalculatorFunction(
                    this.distributeRecurse(new CalculatorFunction(input.leftTerm.leftTerm, input.rightTerm, CalculatorOperator.divide), debug),
                    this.distributeRecurse(new CalculatorFunction(input.leftTerm.rightTerm, input.rightTerm, CalculatorOperator.divide), debug),
                    input.leftTerm.operator
                  );
                case CalculatorOperator.multiply:
                case CalculatorOperator.divide: // (w [*/] x) / (y [+-*/^] z)
            ...
            switch (input.leftTerm.operator) {
              ...
              case CalculatorOperator.subtract:
                return new CalculatorFunction(
                  this.distributeRecurse(new CalculatorFunction(input.leftTerm.leftTerm, input.rightTerm, input.operator), debug),
                  this.distributeRecurse(new CalculatorFunction(input.leftTerm.rightTerm, input.rightTerm, input.operator), debug),
                  input.leftTerm.operator
                );
        ...
        case CalculatorOperator.exponent:
          if (input.leftTerm instanceof CalculatorFunction) {
            if (input.leftTerm.operator === CalculatorOperator.multiply || input.leftTerm.operator === CalculatorOperator.divide) {
              ...
              return new CalculatorFunction(
                this.distributeRecurse(input.leftTerm.leftTerm, debug),
                this.distributeRecurse(new CalculatorFunction(input.leftTerm.rightTerm, input.rightTerm, CalculatorOperator.multiply), debug),
                CalculatorOperator.exponent
              );
            } else return exitWithoutDistributing();
          } else {
