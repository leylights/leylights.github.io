...
function generateArchiveMenu(): void {
  let menuItems: MenuItem[] = Menu.getArchiveMenu();
  ...
  (itemElements[itemElements.length - 1] as HTMLElement).style.boxShadow = "";
}
...
function parallaxScrolls() {
  ...
  if (firstParallax) {
    for (let i = 0; i < showcaseConts.length; i++) {
      (showcaseConts[i] as HTMLElement).style.height = ARCHIVE_SHOWCASE_HEIGHT + "px";
    }
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    (images[i] as HTMLInputElement).style.top = (baseTop * progress) + "px";
  }
}
