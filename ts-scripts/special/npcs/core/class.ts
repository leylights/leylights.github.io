...
type NPCsClassCreationData = {
  ...
  abilities: number[];
  saves: boolean[];
  skillsData: NPCsSkillCreationData[];
  CR: number;
...
export class NPCsClass {
  ...
  constructor(data: NPCsClassCreationData) {
    this.name = data.name;
  ...
  static list: Record<string, NPCsClass> = {
    ...
    masterThief: new NPCsClass({ // https://www.aidedd.org/dnd/monstres.php?vf=maitre-voleur
      name: 'Master Thief',
      armour: NPCsArmour.list.studdedLeather,
  ...
  static getClassByName(name: string): NPCsClass {
    const result: NPCsClass[] = Molasses.Object.values(NPCsClass.list).filter((c: NPCsClass) => {
      return c.name.toLowerCase() === name.toLowerCase();
    });

    if (result.length === 1)
      return result[0];
    else throw new Error('No such class found: ' + name);
  }

  static getRandomClass(): NPCsClass {
    let options: NPCsClass[] = Molasses.Object.values(NPCsClass.list);
