...
export class SideMenuService {
  ...
  static build() {
    ...
    const categories: HTMLElement[] = Array.from(
      SideMenuService.menu.querySelectorAll(".side-menu-dropdown-category")
    );
  ...
  private static createMenuItem(
    ...
    if (!parent) {
      ...
      else parent = document.getElementById("tools-dropdown-category");
    }
    ...
    if (item instanceof MenuItemMulti) {
      ...
      Button.createByAttachment(newItem, () => {
        if (subMenu.style.display !== "") subMenu.style.display = "";
  ...
  private static buildMenuStructure(): HTMLElement {
            ...
            Molasses.createElement({
              ...
              children: [
                Molasses.createElement({
                  type: "img",
                  otherNodes: [
                    { type: "src", value: "/siteimages/closebutton.png" },
                  ],
                }),
              ],
            }),
    ...
    sideMenu
      .querySelector("#side-menu-end-button")
  ...
  private static displayMenuItems(category: HTMLElement) {
    ...
    buttons.forEach((button: HTMLElement) => {
      if (button.style.display !== "block") button.style.display = "block";
      else button.style.display = "none";
    });
  }

  private static generateMenu() {
    MenuLayouts.TOP_MENU.games.forEach((item) => {
      SideMenuService.createMenuItem(item);
    });
    MenuLayouts.TOP_MENU.tools.forEach((item) => {
      SideMenuService.createMenuItem(item);
    });
  }
